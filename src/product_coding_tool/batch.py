...
class ProductBatchCodingAgent:
    ...
    def run(self, request: ProductBatchCodingRequest) -> ProductBatchCodingResult:
        ...
        coding_mode = self._resolve_coding_mode(request)
        logger.info(
            "ProductBatchCodingAgent start rows={} scraped_root={} pg_feature_input={} coding_mode={}",
            len(rows),
        ...
        logger.info(
            "Batch execution plan products={} preflight_failures={} max_parallel_products={} max_parallel_features={} global_llm_concurrency={} coding_mode={}",
        ...
        if max_parallel_products <= 1 or len(preflight_records) <= 1:
            ...
            for record in preflight_records:
                product_result, failure = self._code_one_product(record, request=request, output_root=output_root, coding_mode=coding_mode)
    ...
    def _run_products_parallel(
        ...
        *,
        ...
        with ThreadPoolExecutor(max_workers=max_parallel_products, thread_name_prefix="product_worker") as executor:
            future_to_record = {
                executor.submit(self._code_one_product, record, request=request, output_root=output_root, coding_mode=coding_mode): record
    ...
    def _code_one_product(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                "Coding product input_id={} pg_name={} resolved_pg_name={} artifact={} features={} artifact_files={} missing_expected={} coding_mode={}",
                row.input_id,
                row.pg_name,
            ...
            agent = ProductCodingAgent() if coding_mode == "per_feature" else ProductLevelCodingAgent()
    ...
    def _resolve_coding_mode(self, request: ProductBatchCodingRequest) -> str:
        mode = (request.coding_mode or self.cfg.coding_mode or "per_product").strip().lower()
        if mode in {"product", "bulk", "auto", "per_product"}:
            return "per_product"
        if mode in {"feature", "per_feature"}:
            return "per_feature"
        logger.warning("Unknown coding_mode={!r}; defaulting to per_product", mode)
        return "per_product"
    ...
    def _preflight_llm_if_needed(self, request: ProductBatchCodingRequest) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"LLM preflight failed before product coding started. Original error: {exc}") from exc
